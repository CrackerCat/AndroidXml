...
public class NameSpaceItem {
    ...
    private boolean validate(){
        ...
        if(preOk && uriOk){
            if(!ANDROID_NAME.equals(prefix) && ANDROID_URI.equals(namespaceUri)){
                namespaceUri=APP_URI;
            }
            return true;
        }
        if(!preOk && !uriOk){
    ...
    public static NameSpaceItem getAndroid(){
        if(ns_android==null){
            ns_android=new NameSpaceItem(ANDROID_NAME, ANDROID_URI);
        }
        return ns_android;
    }
    public static NameSpaceItem getApp(){
        if(ns_app==null){
            ns_app=new NameSpaceItem(APP_NAME, APP_URI);
        }
        return ns_app;
    }
